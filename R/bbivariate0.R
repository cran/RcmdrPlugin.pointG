bbivariate0<-function(){
    require("car")
    initializeDialog(title=gettextRcmdr("Graphiques bivariees en saucisson"))
...
onOK <- function(){
        ...
        if (length(x) == 0 || length(y) == 0){
            errorCondition(recall=bbivariate0, message=gettextRcmdr("You must select two variables"))
            return()
            }
