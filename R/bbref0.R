bbref0<-function () 
{
    ...
    initializeDialog(title = gettextRcmdr("Bref."))
    ...
    onOK <- function() {
        if (!is.valid.name(newName)) {
            errorCondition(recall = bbref0, message = paste("\"", 
        ...
        selectVars <- if (tclvalue(allVariables) == "1") 
            ...
            if (0 == length(x)) {
                errorCondition(recall = bbref0, message = gettextRcmdr("No variables were selected."))
                return()
            }
