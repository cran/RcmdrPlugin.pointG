bbisummarize0<-function(){
    initializeDialog(title=gettextRcmdr("Statistiques bivariees en saucisson"))
...
onOK <- function(){
        ...
        if (length(x) == 0 || length(y) == 0){
            errorCondition(recall=bbisummarize0, message=gettextRcmdr("You must select two variables"))
            return()
            }
...
newn <- "D1"
    ...
    OKCancelHelp(helpSubject="bisummarize0")
