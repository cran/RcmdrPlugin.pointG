mmultivariate0<-function(){
    ...
    initializeDialog(title=gettextRcmdr("Factoriel multivariee"))
    
    variablesFrame <- tkframe(top)
    xBox <- variableListBox(variablesFrame, Variables(),selectmode="multiple",title=gettextRcmdr("Variables actives"))
    yBox <- variableListBox(variablesFrame, Variables(),selectmode="multiple", title=gettextRcmdr("Variables supplementaires"))
...
print(y)
        closeDialog()
        if (length(x) == 0){
            errorCondition(recall=mmultivariate0, message=gettextRcmdr("Variables actives non selectionnees"))
            return()
            }
...
return()
    ...
    OKCancelHelp(helpSubject="multivariate0")
