...
bbbivariate0<- function () {
	...
	initializeDialog(title = gettextRcmdr("Graphiques croises en saucisson"))
	.numeric <- Numeric()
	...
	onOK <- function(){
        ...
        if (length(x) == 0 || length(y) == 0){
            errorCondition(recall=bbbivariate0, message=gettextRcmdr("You must select two variables"))
            return()
            }
if (length(x) > 1){
            errorCondition(recall=bbbivariate0, message=gettextRcmdr("Une seule variable explicative"))
            return()
            }
if (length(y) <2){
            errorCondition(recall=bbbivariate0, message=gettextRcmdr("Une tranche contient au moins deux variables"))
            return()
            }
